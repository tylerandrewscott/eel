...
invisible(lapply(seq_along(collaborative_networks), function(x) collaborative_networks[[x]] %v% 'l2_type' <<- c(rep(NA,collaborative_networks[[x]]$gal$bipartite),
                                                                                                                ifelse({collaborative_networks[[x]] %v% 'ONMS_Staff'}[(collaborative_networks[[x]]$gal$bipartite+1):collaborative_networks[[x]]$gal$n] == 1,'Staff',
                                                                                                                ifelse({collaborative_networks[[x]] %v% 'Agency'}[(collaborative_networks[[x]]$gal$bipartite+1):collaborative_networks[[x]]$gal$n] == 1, 'Agency','Other')))))

invisible(lapply(seq_along(technical_networks), function(x) technical_networks[[x]] %v% 'l2_type' <<- c(rep(NA,technical_networks[[x]]$gal$bipartite),
                                                                                                                ifelse({technical_networks[[x]] %v% 'ONMS_Staff'}[(technical_networks[[x]]$gal$bipartite+1):technical_networks[[x]]$gal$n] == 1,'Staff',
                                                                                                                       ifelse({technical_networks[[x]] %v% 'Agency'}[(technical_networks[[x]]$gal$bipartite+1):technical_networks[[x]]$gal$n] == 1, 'Agency','Other')))))
...
collab_sims = parLapply(cl, seq_along(collaborative_networks), function(net) {
   ...
   one_mode_prj = lapply(seq_along(sims),function(x) bipartite::projecting_tm(t(as.sociomatrix(sims[[x]])),method = 'Newman'))
   spread_prj = lapply(seq_along(one_mode_prj),function(x) spread(one_mode_prj[[x]],j,w))
   centralization = sapply(seq_along(one_mode_prj),function(x) sna::centralization(spread_prj[[x]][,-1],degree,mode = 'graph',g = 1, normalize = T,ignore.eval = F)) 
   centralization_agency = sapply(seq_along(one_mode_prj),function(x) sna::centralization(spread_prj[[x]][,-1],degree,mode = 'graph',g = 1, normalize = T,ignore.eval = F,nodes = which(sims[[x]] %v% 'l2_type' == 'Agency') - sims[[x]]$gal$bipartite))  
   centralization_staff = sapply(seq_along(one_mode_prj),function(x) sna::centralization(spread_prj[[x]][,-1],degree,mode = 'graph',g = 1, normalize = T,ignore.eval = F,nodes = which(sims[[x]] %v% 'l2_type' == 'Staff') - sims[[x]]$gal$bipartite))  
   centralization_other = sapply(seq_along(one_mode_prj),function(x) sna::centralization(spread_prj[[x]][,-1],degree,mode = 'graph',g = 1, normalize = T,ignore.eval = F,nodes = which(sims[[x]] %v% 'l2_type' == 'Other') - sims[[x]]$gal$bipartite))  

   betweenness = sapply(seq_along(one_mode_prj),function(x) sna::centralization(spread_prj[[x]][,-1],betweenness,mode = 'graph',g = 1, normalize = T,ignore.eval = F)) 
   betweenness_agency = sapply(seq_along(one_mode_prj),function(x) sna::centralization(spread_prj[[x]][,-1],betweenness,mode = 'graph',g = 1, normalize = T,ignore.eval = F,nodes = which(sims[[x]] %v% 'l2_type' == 'Agency') - sims[[x]]$gal$bipartite))  
   betweenness_staff = sapply(seq_along(one_mode_prj),function(x) sna::centralization(spread_prj[[x]][,-1],betweenness,mode = 'graph',g = 1, normalize = T,ignore.eval = F,nodes = which(sims[[x]] %v% 'l2_type' == 'Staff') - sims[[x]]$gal$bipartite))  
   betweenness_other = sapply(seq_along(one_mode_prj),function(x) sna::centralization(spread_prj[[x]][,-1],betweenness,mode = 'graph',g = 1, normalize = T,ignore.eval = F,nodes = which(sims[[x]] %v% 'l2_type' == 'Other') - sims[[x]]$gal$bipartite))  
   
   closeness = sapply(seq_along(one_mode_prj),function(x) sna::centralization(spread_prj[[x]][,-1],closeness,mode = 'graph',g = 1, normalize = T,ignore.eval = F)) 
   closeness_agency = sapply(seq_along(one_mode_prj),function(x) sna::centralization(spread_prj[[x]][,-1],closeness,mode = 'graph',g = 1, normalize = T,ignore.eval = F,nodes = which(sims[[x]] %v% 'l2_type' == 'Agency') - sims[[x]]$gal$bipartite))  
   closeness_staff = sapply(seq_along(one_mode_prj),function(x) sna::centralization(spread_prj[[x]][,-1],closeness,mode = 'graph',g = 1, normalize = T,ignore.eval = F,nodes = which(sims[[x]] %v% 'l2_type' == 'Staff') - sims[[x]]$gal$bipartite))  
   closeness_other = sapply(seq_along(one_mode_prj),function(x) sna::centralization(spread_prj[[x]][,-1],closeness,mode = 'graph',g = 1, normalize = T,ignore.eval = F,nodes = which(sims[[x]] %v% 'l2_type' == 'Other') - sims[[x]]$gal$bipartite))  
   if(clusters)
   {clustering = sapply(seq_along(sims),function(x) bipartite::clustering_tm(as.sociomatrix(sims[[x]])))

   df = data.frame(centralization,centralization_agency,centralization_staff,centralization_other,
                   betweenness,betweenness_agency,betweenness_staff,betweenness_other,
                   closeness,closeness_agency,closeness_staff,closeness_other,clustering)}
   if(!clusters)
   {
     df = data.frame(centralization,centralization_agency,centralization_staff,centralization_other,
                     betweenness,betweenness_agency,betweenness_staff,betweenness_other,
                     closeness,closeness_agency,closeness_staff,closeness_other)}
  df})
          
if(!clusters)
{saveRDS(collab_sims,'scratch/collab_simulation_results_nocluster.rds')}
if(clusters)
...
tech_sims = parLapply(cl, seq_along(technical_networks), function(net) {
  ...
  #one_mode_prj = lapply(seq_along(sims),function(x) bipartite::projecting_tm(t(as.sociomatrix(sims[[x]])),method = 'Newman'))
  #spread_prj = lapply(seq_along(one_mode_prj),function(x) spread(one_mode_prj[[x]],j,w))
  centralization = sapply(seq_along(sims),function(x) sna::centralization(as.sociomatrix(sims[[x]],'count'),degree,mode = 'graph',g = 1, normalize = T,ignore.eval = F)) 
  centralization_agency = sapply(seq_along(sims),function(x) sna::centralization(as.sociomatrix(sims[[x]],'count'),degree,mode = 'graph',g = 1, normalize = T,ignore.eval = F,nodes = which(sims[[x]] %v% 'l2_type' == 'Agency') - sims[[x]]$gal$bipartite))  
  centralization_staff = sapply(seq_along(sims),function(x) sna::centralization(as.sociomatrix(sims[[x]],'count'),degree,mode = 'graph',g = 1, normalize = T,ignore.eval = F,nodes = which(sims[[x]] %v% 'l2_type' == 'Staff') - sims[[x]]$gal$bipartite))  
  centralization_other = sapply(seq_along(sims),function(x) sna::centralization(as.sociomatrix(sims[[x]],'count'),degree,mode = 'graph',g = 1, normalize = T,ignore.eval = F,nodes = which(sims[[x]] %v% 'l2_type' == 'Other') - sims[[x]]$gal$bipartite))  

  betweenness = sapply(seq_along(sims),function(x) sna::centralization(as.sociomatrix(sims[[x]],'count'),betweenness,mode = 'graph',g = 1, normalize = T,ignore.eval = F)) 
  betweenness_agency = sapply(seq_along(sims),function(x) sna::centralization(as.sociomatrix(sims[[x]],'count'),betweenness,mode = 'graph',g = 1, normalize = T,ignore.eval = F,nodes = which(sims[[x]] %v% 'l2_type' == 'Agency') - sims[[x]]$gal$bipartite))  
  betweenness_staff = sapply(seq_along(sims),function(x) sna::centralization(as.sociomatrix(sims[[x]],'count'),betweenness,mode = 'graph',g = 1, normalize = T,ignore.eval = F,nodes = which(sims[[x]] %v% 'l2_type' == 'Staff') - sims[[x]]$gal$bipartite))  
  betweenness_other = sapply(seq_along(sims),function(x) sna::centralization(as.sociomatrix(sims[[x]],'count'),betweenness,mode = 'graph',g = 1, normalize = T,ignore.eval = F,nodes = which(sims[[x]] %v% 'l2_type' == 'Other') - sims[[x]]$gal$bipartite))  
  
  closeness = sapply(seq_along(sims),function(x) sna::centralization(as.sociomatrix(sims[[x]],'count'),closeness,mode = 'graph',g = 1, normalize = T,ignore.eval = F)) 
  closeness_agency = sapply(seq_along(sims),function(x) sna::centralization(as.sociomatrix(sims[[x]],'count'),closeness,mode = 'graph',g = 1, normalize = T,ignore.eval = F,nodes = which(sims[[x]] %v% 'l2_type' == 'Agency') - sims[[x]]$gal$bipartite))  
  closeness_staff = sapply(seq_along(sims),function(x) sna::centralization(as.sociomatrix(sims[[x]],'count'),closeness,mode = 'graph',g = 1, normalize = T,ignore.eval = F,nodes = which(sims[[x]] %v% 'l2_type' == 'Staff') - sims[[x]]$gal$bipartite))  
  closeness_other = sapply(seq_along(sims),function(x) sna::centralization(as.sociomatrix(sims[[x]],'count'),closeness,mode = 'graph',g = 1, normalize = T,ignore.eval = F,nodes = which(sims[[x]] %v% 'l2_type' == 'Other') - sims[[x]]$gal$bipartite))  
  if(clusters)
  {clustering = sapply(seq_along(sims),function(x) bipartite::clustering_tm(as.sociomatrix(sims[[x]])))
...
if(!clusters)
{saveRDS(tech_sims,'scratch/tech_simulation_results_nocluster.rds')}
if(clusters)
