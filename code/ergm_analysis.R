...
collab_ergms = parLapply(cl,seq_along(collaborative_networks), function(net) {try(ergm(collaborative_networks[[net]]~edges + b1degree(0) + b2degree(0) +
...
saveRDS(collab_ergms,'mod_ergm_collab.rds')

tech_ergms = parLapply(cl,seq_along(technical_networks), function(net) {try(ergm(technical_networks[[net]]~edges + b1degree(0) + b2degree(0) +
                                                                              gwb1degree(gwb1_alpha ,fixed=T) + gwb2degree(gwb2_alpha ,fixed=T) + 
                                                                              b1cov('Role_Count') + b1cov('Time_Involved') + b1factor('On_ONMS_Staff') ,
                                                                            eval.loglik = F,verbose=F,constraints = ~edges,
                                                                            control = control.ergm(MCMC.interval = interval,MCMC.burnin = burnin,
                                                                                                   MCMC.samplesize = sample)))})

saveRDS(tech_ergms,'mod_ergm_tech.rds')
stopCluster(cl)

readRDS('scratch/tech_poisson.rds')
